...
if __name__ == '__main__':
  ...
  dataset = getattr(data_utils, dataset_name)(score_dir, vocab_path, key_aug=data_param.key_aug, vocab_name=net_param.vocab_name, num_limit=100)
  ...
  # if args.model_type == 'pitch_dur':
  #   dataset = PitchDurSplitSet(score_dir, vocab_path, key_aug=args.aug_type)
  #   model = PitchDurModel(dataset.vocab.get_size(), net_param)
  # elif args.model_type == 'measure_info':
  #   dataset = MeasureOffsetSet(score_dir, vocab_path, key_aug=args.aug_type)
  #   model = MeasureInfoModel(dataset.vocab.get_size(), net_param)
  # elif args.model_type == 'measure_hier':
  #   dataset = MeasureNumberSet(score_dir, vocab_path, key_aug=args.aug_type)
  #   model = MeasureHierarchyModel(dataset.vocab.get_size(), net_param)
  # elif args.model_type == 'vanilla':
  #   dataset = ABCset(score_dir, vocab_path, key_aug=args.aug_type)    
  #   model = LanguageModel(len(dataset.vocab), net_param)
  # else:
  ...
  if not args.no_log:
    wandb.init(project="title-embedding", entity="clayryu", config={**vars(args), **config})
