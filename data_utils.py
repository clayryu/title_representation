...
class ABCsetTitle(MeasureOffsetSet):
  def __init__(self, dir_path, vocab_path, vocab_name, num_limit=100):
    #super().__init__(dir_path, vocab_path)
    if isinstance(dir_path, str) or isinstance(dir_path, Path):
      self.dir = Path(dir_path)
      self.abc_list = list(self.dir.rglob('*.abc')) + list(self.dir.rglob('*.ABC'))
      if num_limit is not None:
        self.abc_list = self.abc_list[:num_limit]
      self.tune_list, error_list, self.filtered_tunes_list, self.title_in_text = prepare_abc_title(self.abc_list)
    elif isinstance(dir_path, FolkRNNSet):
      self.tune_list = dir_path.pyabc_tunes
    else:
      print(f'Error: Unknown input type: {type(dir_path)}')
  ...
  def _prepare_data(self):
    self.data = [self._tune_to_list_of_str(tune) for tune in self.tune_list if tune.is_tune_with_full_measures]
    self.header = [tune.header for tune in self.tune_list if tune.is_tune_with_full_measures]
    self.filtered_tunes_list_avail = []
    self.title_in_text_avail = []
    for tunes, title in zip(self.filtered_tunes_list, self.title_in_text):
      filtered_tunes = []
      for tune in tunes:
      ...
      if len(filtered_tunes) > 0:
        self.filtered_tunes_list_avail.append(filtered_tunes)
        self.title_in_text_avail.append(title)
  ...
  def __len__(self):
    return len(self.filtered_tunes_list_avail)
      
  def __getitem__(self, idx):
    filtered_tunes = self.filtered_tunes_list_avail[idx]
    tune_sample = random.sample(filtered_tunes, k=1)[0]
    tune_data, tune_header = self.prepare_tune(tune_sample)
    
    tune = [['<start>','<start>','<start>' ]] + tune_data + [['<end>', '<end>', '<end>']]
    header = tune_header
    tune_in_idx = [self.vocab(token) for token in tune]
